...
def _log_llm_call(player_name, role, action, result):
    """记录 LLM 调用"""
    global LLM_CALL_LOG
    entry = {
    ...
    if len(LLM_CALL_LOG) > 20:
        LLM_CALL_LOG.pop(0)
    print(f"[LLM] {entry['time']} {player_name}({role}) {action}: {result[:100] if result else 'None'}...")
...
【思考框架·救人】第一夜尽量救人（除非被刀的是确定狼）。如果被刀的是自己，必须自救！后期判断被刀者身份价值。
【思考框架·毒人】只有非常确定某人是狼才毒，毒错好人等于送狼一轮。毒药不要轻易用，留着关键时刻追轮次。
【Few-shot 示例1】场景:第1夜,3号被刀,你有解药。思考:第1夜信息极少,3号身份未知但第一天就死太亏,用解药救人。不用毒药。<answer>heal:3号, poison:不毒</answer>
【Few-shot 示例2】场景:第2夜,你自己被刀,你有解药。思考:我被刀了！必须自救,否则女巫死了好人两瓶药都没了。<answer>heal:{name}, poison:不毒</answer>
【Few-shot 示例3】场景:第3夜,5号被刀(5号是预言家金水),你有毒药,7号是狼查杀。思考:5号是金水要救,7号是确定狼直接毒掉。<answer>heal:5号, poison:7号</answer>
请做出决定:""",
}

# ====================== 投票决策 Few-shot 推理模板 ======================
VOTE_PROMPTS = {
    "werewolf": """你是狼人杀中的狼人\"{name}\"，狼队友:{wolf_team}。你需要投票。
【重要】只投存活的好人！绝对不能投票给死亡玩家！死亡玩家已经不能投票了！
狼人策略：票投好人，绝不投队友。
【Few-shot 示例】场景:预言家5号查杀了你队友3号（3号已死）。思考:3号已死投不了，投其他好人。<answer>先知</answer>
场景:只有1号2号存活，1号是队友。思考:只能投2号。<answer>2号</answer>
请选择投票目标:""",

    "seer": """你是狼人杀中的预言家\"{name}\"。你需要投票。
【重要】只投存活的狼人！死亡玩家不能投票！
你的票必须投给你查验出的狼人或最可疑的人。
【Few-shot 示例】场景:你昨晚查杀7号（7号已死）。思考:7号已死投不了，投其他狼人。<answer>3号</answer>
请选择投票目标:""",

    "good": """你是狼人杀中的好人\"{name}\"（{role_name}）。你需要投票。
【重要】只投存活玩家！死亡玩家已经不能投票了！
好人策略：找狼、票狼。
【投票思考框架】1.谁被预言家查杀了？→首选投查杀（查杀还活着的话）2.谁的发言逻辑最不连贯？→摇摆、前后矛盾的人像狼 3.死亡玩家不能投票！
【Few-shot 示例】场景:5号报查杀7号（7号已死）。思考:7号已死投不了，找下一个可疑的人。<answer>3号</answer>
请选择投票目标:""",
}
...
@app.route("/api/debug/llm_log", methods=["GET"])
def api_debug_llm_log():
    """调试接口：查看LLM调用日志"""
    return jsonify({"log": LLM_CALL_LOG, "USE_LLM": USE_LLM})
...
def api_debug_rooms():
    ...
    for rid, room in rooms.items():
        result[rid] = {
            "phase": room.phase,
            "day": room.day,
            "players": [{"name": p.name, "role": p.role, "alive": p.alive, "is_human": p.is_human} for p in room.players],
            "night_history": room.night_history if hasattr(room, 'night_history') else [],
            "vote_history": room.vote_history if hasattr(room, 'vote_history') else [],
        }
...
def api_add_ai_preset(room_id):
    ...
    # 特殊人设：none=无风格, mystery_guest=神秘嘉宾
    if persona_key == "none":
        persona_key = None  # 不设置人设，纯理性
    elif persona_key == "mystery_guest":
        # 神秘嘉宾：随机人设但名字特殊
        pass  # persona_key 保持 "mystery_guest"，在名字池中选用特殊名
    elif persona_key and persona_key not in AI_PERSONAS:
...
def api_list_personas():
    ...
    for key, val in AI_PERSONAS.items():
        frontend_personas[key] = {
            "name": val.get("name", key),
            "desc": val.get("style", ""),
            "tag": "persona",
        }
    frontend_personas["mystery_guest"] = {
        "name": "🎭 神秘嘉宾",
        "desc": "特邀嘉宾空降，身份成谜",
        "tag": "special",
    }
    return jsonify({"personas": frontend_personas})
...
def _phase_seer(room_id, token):
    ...
    if seer.is_ai:
        ...
        if alive:
            ...
            print(f"[LLM] 预言家决策 - 可验目标: {[p.name for p in alive]}, LLM返回: {llm_target}")
            if llm_target:
    ...
    if witch.is_ai:
        ...
        if USE_LLM:
            try:
                ...
                print(f"[LLM] 女巫决策 - kill_target={kill_target}, can_heal={witch.witch_heal}, can_poison={witch.witch_poison}")
                print(f"[LLM] 女巫决策结果: {result}")
                if result:
...
def _call_deepseek(system_prompt, user_prompt, max_tokens=200):
    """调用 DeepSeek API,返回文本。失败返回 None。"""
    if not USE_LLM:
        print("[LLM] USE_LLM=False, 跳过调用")
        return None
    try:
        import urllib.request
        print(f"[LLM] 调用 DeepSeek, user_prompt 长度={len(user_prompt)}")
        payload = json.dumps({
            "model": "deepseek-chat",
            "max_tokens": max_tokens,
            "temperature": 0.9,
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            print(f"[LLM] DeepSeek 返回: {result[:100]}..." )
            return result
    except Exception as e:
...
def _call_deepseek_logged(player_name, role, action, system_prompt, user_prompt, max_tokens=200):
    """调用 DeepSeek 并记录日志"""
    result = _call_deepseek(system_prompt, user_prompt, max_tokens)
    _log_llm_call(player_name, role, action, result)
    return result
...
【本轮发言】<给其他玩家看的发言,20-50字,口语化,有站边有理由>"""
    ...
    # 尝试调用 LLM
    llm_result = _call_deepseek_logged(player.name, role_name, "发言", sys_prompt, user_prompt, max_tokens=400)
    if llm_result:
        import re as _re
...
请选择投票目标:"""
    ...
    result = _call_deepseek_logged(player.name, ROLES.get(player.role, {}).get('name', player.role), "投票", sys_prompt, user_prompt, max_tokens=150)
    if result:
